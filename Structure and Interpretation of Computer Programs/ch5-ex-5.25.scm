...
;; Lazy-evaluation
(define (thunk? obj)
  (and (pair? obj) (tagged-list? obj 'thunk)))
(define (thunk-exp thunk) (cadr thunk))
(define (thunk-env thunk) (caddr thunk))
(define (evaluated-thunk? obj)
  (and (pair? obj) (tagged-list? obj 'evaluated-thunk)))
...
(define eceval
  (make-machine
   ...
   '(
     ...
     ;; Main lazy-evaluation logic
     (save proc)
     (test (op primitive-procedure?) (reg proc))
     (branch (label ev-appl-arg-values-loop))
     ;; Delayed args
     ev-appl-delayed-args-loop
     (test (op no-operands?) (reg unev))
     (branch (label ev-appl-after-accum-delayed-args))
     (assign exp (op first-operand) (reg unev))
     (assign exp (op delay-it) (reg exp) (reg env))
     (assign argl (op adjoin-arg) (reg exp) (reg argl))
     (assign unev (op rest-operands) (reg unev))
     (goto (label ev-appl-delayed-args-loop))
     ev-appl-after-accum-delayed-args
     (restore proc)
     (goto (label apply-dispatch))
     ;; Arg values
     ev-appl-arg-values-loop
     (save argl)
     (save env)
     (save unev)
     (assign exp (op first-operand) (reg unev))
     (test (op last-operand?) (reg unev))
     (branch (label ev-appl-accum-last-arg-value))
     ...
     (assign continue (label after-force-it-thunk))
     (goto (label actual-value))
     after-force-it-thunk
     (restore proc)
     (perform (op set-car!) (reg proc) (const evaluated-thunk))
     (assign proc (op cdr) (reg proc))
     (perform (op set-car!) (reg proc) (reg val))
     (perform (op set-cdr!) (reg proc) (const ()))
     ...
     (assign unev (op rest-operands) (reg unev))
     (goto (label ev-appl-arg-values-loop))
     ...
     (save env)
     (save unev)
     (assign continue (label ev-appl-after-accum-last-arg-value))
     (goto (label actual-value))
     ev-appl-after-accum-last-arg-value
     (restore unev)
     (restore env)
     (restore argl)
     (assign argl (op adjoin-arg) (reg val) (reg argl))
