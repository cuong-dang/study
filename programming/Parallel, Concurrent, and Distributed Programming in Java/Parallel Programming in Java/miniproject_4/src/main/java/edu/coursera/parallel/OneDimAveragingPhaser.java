...
public final class OneDimAveragingPhaser {
    ...
    public static void runParallelFuzzyBarrier(final int iterations,
        ...
        for (int ii = 0; ii < tasks; ++ii) {
            int i = ii;
            threads[ii] = new Thread(() -> {
                double[] tempNew = myNew, tempVal = myVal;
                for (int iter = 0; iter < iterations; ++iter) {
                    int left = i * (n / tasks) + 1;
                    tempNew[left] = (tempVal[left - 1] + tempVal[left + 1]) / 2.0;

                    int right = (i + 1) * (n / tasks);
                    tempNew[right] = (tempVal[right - 1] + tempVal[right + 1]) / 2.0;

                    int currentPhase = ph.arrive();

                    for (int j = left + 1; j <= right - 1; ++j)
                        tempNew[j] = (tempVal[j - 1] + tempVal[j + 1]) / 2.0;
