...
public abstract class PageTuple implements Tuple {
    ...
    private void setNullColumnValue(int iCol) {
        ...
        int len = getColumnValueSize(schema.getColumnInfo(iCol).getType(), offset);
    ...
    private void setNonNullColumnValue(int iCol, Object value) {
        ...
        if (colType.getBaseType() == SQLDataType.VARCHAR && !isNullValue(iCol)) {
            int oldlen = getColumnValueSize(colType, valueOffsets[iCol]);
            int newlen = getStorageSize(colType, TypeConverter.getStringValue(value).length());
            if (oldlen == newlen) {
            ...
            int d = oldlen - newlen;
            if (oldlen > newlen) {
                deleteTupleDataRange(valueOffsets[iCol], d);
            } else {
                insertTupleDataRange(valueOffsets[iCol], -d);
            }
            writeNonNullValue(dbPage, valueOffsets[iCol] + d, colType, value);
            pageOffset += d;
            computeValueOffsets();
            return;
        }
        ...
        if (isNullValue(iCol)) {
            int len = getStorageSize(colType, TypeConverter.getStringValue(value).length());
            int insertPos = iCol == valueOffsets.length - 1 ? endOffset : valueOffsets[iCol + 1];
            insertTupleDataRange(insertPos, len);
            writeNonNullValue(dbPage, insertPos - len, colType, value);
            pageOffset -= len;
