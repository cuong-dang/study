...
public class DataPage {
    ...
    public static void deleteTuple(DBPage dbPage, int slot) {
        ...
        /* Delete slot data */
        int tupleLen = getTupleLength(dbPage, slot);
        int tupleOff = getSlotValue(dbPage, slot);
        deleteTupleDataRange(dbPage, tupleOff, tupleLen);
        setSlotValue(dbPage, slot, EMPTY_SLOT);

        /* Attempt to reclaim empty slots */
        int endingSlot = slot;
        while (endingSlot < numSlots - 1) {
            if (getSlotValue(dbPage, endingSlot + 1) == EMPTY_SLOT) {
                endingSlot++;
            } else {
                break;
            }
        }
        if (endingSlot == numSlots - 1) {
            int startingSlot = slot;
            while (startingSlot > 0) {
                if (getSlotValue(dbPage, startingSlot - 1) == EMPTY_SLOT) {
                    startingSlot--;
                } else {
                    break;
                }
            }
            int numSlotsToReclaim = endingSlot - startingSlot + 1;
            setNumSlots(dbPage, numSlots - numSlotsToReclaim);
        }
    }
}
