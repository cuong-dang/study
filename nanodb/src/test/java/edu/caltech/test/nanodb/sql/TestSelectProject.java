...
public class TestSelectProject extends SqlTestCase {
    ...
    public void testFromSubquery() throws Throwable {
        TupleLiteral[] expected = { new TupleLiteral(1) };
        CommandResult result = server.doCommand("SELECT id FROM (SELECT 1 AS id) t", true);
        assert checkUnorderedResults(expected, result);
    }
}
