...
public class TestSimpleSelects extends SqlTestCase {
    ...
    public void testSelectLiterals() throws Throwable {
        TupleLiteral[] expected1 = {new TupleLiteral(1)};
        CommandResult result = server.doCommand("SELECT 1", true);
        assert checkUnorderedResults(expected1, result);

        TupleLiteral[] expected2 = {new TupleLiteral(2)};
        result = server.doCommand("SELECT 1+1", true);
        assert checkUnorderedResults(expected2, result);

        TupleLiteral[] expected3 = {new TupleLiteral(1, 6)};
        result = server.doCommand("SELECT 1, 2*3", true);
        assert checkUnorderedResults(expected3, result);
    }
}
